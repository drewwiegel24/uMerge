...
class chatbot():
    ...
    """Work in progress"""
    def add_randomness_to_context_sections(self, sorted_contexts: List[Tuple[float, Tuple[str, str]]], degree: int) -> List[Tuple[float, Tuple[str, str]]]:
        denominator = 0.5 * degree * (degree + 1)
        print(denominator)
        sorted_with_randomness = sorted_contexts
        return sorted_with_randomness
        

    def construct_prompt(self, question: str, query_embeddings: dict, context_embeddings: dict, df: pd.DataFrame, separator_len: int) -> str:
        ...
        # print(f"Selected {len(chosen_sections)} context sections:")
    ...
    def answer_query_with_context(self, question: str, query_embeddings: dict, df: pd.DataFrame, context_embeddings: dict, separator_len: int, show_prompt: bool,) -> str:
        prompt = self.construct_prompt(question, query_embeddings, context_embeddings, df, separator_len)
    ...
    def static_chatbot(self):
        ...
        for key in self.club_query_embeddings:
            response = self.answer_query_with_context(key, self.club_query_embeddings[key], self.club_data_df, self.club_contextual_embeddings, self.separator_len, show_prompt=False)
            print(response + "\n===\n")
        ...
        for key in self.event_query_embeddings:
            response = self.answer_query_with_context(key, self.event_query_embeddings[key], self.event_data_df, self.event_contextual_embeddings, self.separator_len, show_prompt=False)
            print(response + "\n===\n")

        print(self.relevant_ids)

        #Want to get to a point where can ask a general question and get answers related to clubs and/or events
        #all_contextual_embeddings = club_contextual_embeddings.update(event_contextual_embeddings)
        print("\n\n\n")
    ...
    def live_chatbot(self):
        ...
        while query != "Exit":
            ...
            #event_and_club_embeddings = {**self.event_contextual_embeddings, **self.club_contextual_embeddings}
            relevant_query_contexts = self.order_context_sections_by_query(query_embedding, self.event_contextual_embeddings)
            self.event_data_df.set_index('Name', inplace=True)
            #event_and_club_data_df = self.event_data_df[["Name", "Description", "ID", "Tokens"]].append(self.club_data_df[["Name", "Description", "ID", "Tokens"]])
            response = self.answer_query_with_context(query, query_embedding, self.event_data_df, self.event_contextual_embeddings, self.separator_len, show_prompt=False)
            print(response + "\n===\n")
    ...
    def get_relevent_event_ids(self, query: str, num_ids = -1) -> pd.DataFrame:
        ...
        for embedding, index in relevant_query_contexts:
            ...
            ordered_ids_df.loc[len(ordered_ids_df.index)] = self.event_data_df.iloc[index].loc[['ID', 'Name']]
    ...
    def filter_dates(self, startDate: datetime, endDate: datetime):
        return None
    ...
    def get_relevant_combined_ids(self, query: str, num_ids = -1, startDate = None, endDate = None) -> pd.DataFrame:
        ...
        # for row in ordered_ids_df.iterrows():
        #     print(row[1]['Date'])
        # if startDate != None and endDate != None:
        #     self.filter_dates(startDate, endDate)
...
if __name__ == "__main__":
    chatbot_1 = chatbot()
    chatbot_1.static_chatbot()
    #chatbot_1.live_chatbot()
    #chatbot_1.get_relevent_event_ids("What clubs are related to sports?", 10)
    #chatbot_1.get_relevent_club_ids("What clubs are related to sports?", 10)

    #chatbot_1.get_relevant_combined_ids("Tell me about events related to biology.", 10)

    #sorted_contexts = chatbot_1.order_context_sections_by_query(chatbot_1.event_query_embeddings, chatbot_1.event_contextual_embeddings)
    #chatbot_1.add_randomness_to_context_sections(sorted_contexts, 5)
